...
class env(bew.base_env):
    ...
    def __init__(self, env_name, rand_seed, misc_info):
        ...
        if 'reset_type' in misc_info and misc_info['reset_type'] == 'gym':
            ...
            # self.observation_space = spaces.Box(low=-1, high=1, shape=(self._env_info['ob_size'],), dtype=np.float32)
            ...
            self.reward_range = [-16.2736044, 0]  # https://github.com/openai/gym/wiki/Pendulum-v0
            self.metadata = {}
        else:
            self._reset_return_obs_only = False

        self._max_episode_steps = self._env_info['max_length']
    ...
    def reset(self, control_info={}):
        ...
        # my comment: I do not understand why the reset for the 2nd time and there's no mujoco here
    ...
    def _set_reward_api(self):
        ...
        def reward(data_dict):
            ...
            theta = np.arctan2(x, y) # y = cos(theta), x = sin(theta)
            #costs = y + .1 * np.abs(x) + .1 * (thetadot ** 2) + .001 * (torque ** 2)
            costs = angle_normalize(theta) ** 2 + .1 * thetadot ** 2 + .001 * (torque ** 2)
        ...
        def reward_derivative_bk(data_dict, target):
            y_ob_pos = 0
            x_ob_pos = 1
            thetadot_ob_pos = 2
        ...
        def reward_derivative(data_dict, target):
            ...
            num_data = len(data_dict['start_state'])
            if target == 'state':
                derivative_data = np.zeros([num_data, 2], dtype=np.float)
                derivative_data[:, theta_pos] += - 2*data_dict['start_date'][:, theta_pos]
                derivative_data[:, thetadot_ob_pos] += -0.2 * data_dict['start_date'][:, thetadot_ob_pos]
                ...
                derivative_data = np.zeros(
                    [num_data, self._env_info['action_size'], 2],
                    dtype=np.float
                )

            elif target == 'state-action':
                derivative_data = np.zeros(
                    [num_data, 2, self._env_info['action_size']],
                    dtype=np.float
                )
